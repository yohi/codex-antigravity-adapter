...
- [ ] 2.4 エラー処理とレスポンス正規化機能を実装
  - ネットワーク接続失敗時の標準エラーレスポンス生成
  - タイムアウト発生時の標準エラーレスポンス生成
  - 予期しない例外発生時の標準エラーレスポンス生成
  - 上位サーバーエラーの透過的な返却処理を実装
